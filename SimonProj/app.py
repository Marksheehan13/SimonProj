...
class RequestIdFilter(logging.Filter):
    def filter(self, record):
        record.request_id = getattr(g, "request_id", "N/A") if app else "N/A"
        return True
...
@app.route("/history")
def history():
    """Return recent searches from the database."""
    try:
        db = get_db()
        rows = db.execute(
...
@app.route("/status")
def status():
    """Status page — checks DB and NASA API connectivity."""
    results = {}
